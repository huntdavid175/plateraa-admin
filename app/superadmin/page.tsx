...
const topRestaurants = [
  { rank: 1, name: "Mama Cass", location: "Lagos", orders: 2456, revenue: 5234000 },
  { rank: 2, name: "Chicken Republic", location: "Lagos", orders: 2234, revenue: 4890000 },
  { rank: 3, name: "Amala Spot", location: "Ibadan", orders: 1987, revenue: 4567000 },
  { rank: 4, name: "Mr Biggs", location: "Abuja", orders: 1845, revenue: 4234000 },
  { rank: 5, name: "Tastee Fried", location: "Lagos", orders: 1756, revenue: 3987000 },
  { rank: 6, name: "Buka Express", location: "Lagos", orders: 1678, revenue: 3756000 },
  { rank: 7, name: "Suya Spot", location: "Kano", orders: 1567, revenue: 3456000 },
  { rank: 8, name: "Rice & More", location: "Port H.", orders: 1489, revenue: 3234000 },
  { rank: 9, name: "Naija Kitchen", location: "Enugu", orders: 1398, revenue: 2987000 },
  { rank: 10, name: "Jollof Palace", location: "Lagos", orders: 1287, revenue: 2845000 },
];

const recentActivity = [
  { time: "2 mins ago", icon: "🆕", type: "New restaurant signup", details: "Iya Basira" },
  { time: "5 mins ago", icon: "💰", type: "Plan upgrade", details: "Mama Cass upgraded to Professional" },
  { time: "8 mins ago", icon: "⚠️", type: "Payment failed", details: "Chicken Republic" },
  { time: "12 mins ago", icon: "📦", type: "Milestone", details: "1,234th order today processed" },
  { time: "15 mins ago", icon: "🔧", type: "System", details: "System backup completed" },
  { time: "18 mins ago", icon: "👤", type: "New user", details: "New user registered (Amala Spot)" },
  { time: "25 mins ago", icon: "📊", type: "Report", details: "Daily report sent to 234 restaurants" },
  { time: "30 mins ago", icon: "🎉", type: "Milestone", details: "Buka Express hit 1,000 orders" },
];

const systemHealth = [
  { name: "API Status", status: "operational", value: "45ms avg", color: "emerald" },
  { name: "Database", status: "healthy", value: "12.3GB / 50GB", color: "emerald" },
  { name: "Payment Gateway", status: "connected", value: "", color: "emerald" },
  { name: "SMS Service", status: "active", value: "98.5% delivery", color: "emerald" },
  { name: "Email Service", status: "active", value: "", color: "emerald" },
  { name: "Storage", status: "healthy", value: "234GB / 1TB used", color: "emerald" },
  { name: "Background Jobs", status: "operational", value: "456 processed/hour", color: "emerald" },
  { name: "Error Rate", status: "low", value: "0.03% (last hour)", color: "emerald" },
];
...
export default function SuperAdminPage() {
  return (
    <div className="p-6 space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-[var(--foreground)] mb-2">Platform Overview</h1>
      ...
      <div className="bg-[var(--card)] rounded-xl border border-[var(--border)] p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-[var(--foreground)]">Top Performing Restaurants</h2>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-[var(--border)]">
                ...
                <th className="text-left py-3 px-4 text-xs font-semibold text-[var(--muted-foreground)] uppercase">Location</th>
                <th className="text-right py-3 px-4 text-xs font-semibold text-[var(--muted-foreground)] uppercase">Orders</th>
                <th className="text-right py-3 px-4 text-xs font-semibold text-[var(--muted-foreground)] uppercase">Revenue</th>
              </tr>
            </thead>
            <tbody>
              {topRestaurants.map((restaurant) => (
                <tr key={restaurant.rank} className="border-b border-[var(--border)] hover:bg-[var(--muted)]">
                  ...
                  <td className="py-3 px-4 font-medium text-[var(--foreground)]">{restaurant.name}</td>
                  <td className="py-3 px-4 text-sm text-[var(--muted-foreground)]">{restaurant.location}</td>
                  <td className="py-3 px-4 text-right text-sm text-[var(--foreground)]">{restaurant.orders.toLocaleString()}</td>
                  <td className="py-3 px-4 text-right font-medium text-[var(--foreground)]">₵{(restaurant.revenue / 1000).toFixed(0)}k</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-[var(--card)] rounded-xl border border-[var(--border)] p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-[var(--foreground)]">Platform Activity</h2>
            <button className="text-sm text-[var(--primary)] hover:underline">View All</button>
          </div>
          <div className="space-y-3">
            {recentActivity.map((activity, i) => (
              <div key={i} className="flex items-start gap-3 p-3 rounded-lg hover:bg-[var(--muted)]">
                <span className="text-xl">{activity.icon}</span>
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-[var(--foreground)]">
                    <span className="font-medium">{activity.type}:</span> {activity.details}
                  </p>
        ...
        <div className="bg-[var(--card)] rounded-xl border border-[var(--border)] p-6">
          <h2 className="text-lg font-semibold text-[var(--foreground)] mb-4">System Health</h2>
          <div className="space-y-3">
